...
public class BTreeFile implements DbFile {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		//halfKeyValue = t.getField(keyField); // old code

		Tuple firstOfRight = newLeafPage.iterator().next();
		halfKeyValue = firstOfRight.getField(keyField);
		...
		if (page.getRightSiblingId() != null) {
			...
			dirtypages.put(originRight.getId(), originRight);
		}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int sum = page.getNumTuples() + sibling.getNumTuples();
		int sourceNtups = page.getNumTuples();
		...
		Tuple t = null;
		while (sourceNtups < sum / 2 && it.hasNext()) {
			t = it.next();
			sibling.deleteTuple(t);
			page.insertTuple(t);
			sourceNtups += 1;
		}
		Tuple headTupleOfRight = null;
		if (isRightSibling) {
			headTupleOfRight = sibling.iterator().next();
		} else {
			headTupleOfRight = page.iterator().next();
		}
		entry.setKey(headTupleOfRight.getField(keyField));
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int sum = page.getNumEntries() + leftSibling.getNumEntries() + 1;
		int sourceEntries = page.getNumEntries();

		Iterator<BTreeEntry> leftIterator = leftSibling.reverseIterator();
		Iterator<BTreeEntry> rightIterator = page.iterator();
		BTreeEntry leftEntry = leftIterator.next();
		BTreeEntry rightEntry = rightIterator.next();
		BTreeEntry fromParentEntry = new BTreeEntry(parentEntry.getKey(), leftEntry.getRightChild(), rightEntry.getLeftChild());
		page.insertEntry(fromParentEntry);
		sourceEntries += 1;

		while (sourceEntries < sum / 2) {
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int sum = page.getNumEntries() + rightSibling.getNumEntries() + 1;
		int sourceEntries = page.getNumEntries();

		Iterator<BTreeEntry> rightIterator = rightSibling.iterator();
		Iterator<BTreeEntry> leftIterator = page.reverseIterator();
		BTreeEntry leftEntry = leftIterator.next();
		BTreeEntry rightEntry = rightIterator.next();
		BTreeEntry fromParentEntry = new BTreeEntry(parentEntry.getKey(), leftEntry.getRightChild(), rightEntry.getLeftChild());
		page.insertEntry(fromParentEntry);
